...
namespace Street_Fighter {
    ...
    public class Game1 : Microsoft.Xna.Framework.Game {
        ...
        enum GameState {
            ...
            TitleScreen, InstructionScreen, BattleScreen
        ...
        Song titleScreenMusic, instructionScreenMusic;
        Song[] battleScreenMusic = new Song[3];
        int randBattleScreenMusic;
        Random rand = new Random();

        SoundEffect titleScreenStartSound, instructionScreenReadySound, instructionScreenStartSound;
        SoundEffect narratorFight;
        ...
        Texture2D globe;
        Texture2D titleScreenTitlePic;
        Rectangle globeRect;
        Rectangle titleScreenTitleRect;

        String startMessage = "Press Start or Enter to Begin";
        Vector2 startMessageVector = new Vector2(230, 400);

        bool startMessageVisible = true;

        int titleToInstructionSeparation = 0;

        //Variables for Instructions Screen
        Texture2D xboxControlsMovementPic, xboxControlsAttacksPic;
        Texture2D leftShoulderClickedPic, rightShoulderClickedPic, leftShoulderNotClickedPic, rightShoulderNotClickedPic;
        Rectangle xboxControlsMovementRect, xboxControlsAttacksRect;
        Rectangle p1LeftShoulderRect, p1RightShoulderRect, p2LeftShoulderRect, p2RightShoulderRect;

        bool pad1LeftShoulderPressed = false;
        bool pad1RightShoulderPressed = false;
        bool pad2LeftShoulderPressed = false;
        bool pad2RightShoulderPressed = false;

        bool p1Ready = false;
        bool p2Ready = false;

        int instructionToBattleSeparation = 0;
        ...
        int knockoutSeparation = 60;
        int victorySeparation = 30;
        ...
        protected override void Initialize() {
            ...
            titleScreenTitleRect = new Rectangle(80, 10, 640, 360);

            //Instruction Screen Variables
            globeRect = new Rectangle(0, 0, GraphicsDevice.Viewport.Width, GraphicsDevice.Viewport.Height);
            xboxControlsMovementRect = new Rectangle(10, 170, 185, 172);
            xboxControlsAttacksRect = new Rectangle(185, 170, 210, 195);
            p1LeftShoulderRect = new Rectangle(30, 400, 156, 56);
            p1RightShoulderRect = new Rectangle(210, 400, 156, 56);
            p2LeftShoulderRect = new Rectangle(430, 400, 156, 56);
            p2RightShoulderRect = new Rectangle(610, 400, 156, 56);
            ...
            shoryukenHitboxKen = new Rectangle(-1000, -1000, 100, 250);
            ...
            shoryukenHitboxRyu = new Rectangle(-1000, -1000, 100, 250);
        ...
        protected override void LoadContent() {
            ...
            titleScreenMusic = Content.Load<Song>("music/title_screen_music");
            instructionScreenMusic = Content.Load<Song>("music/instruction_screen_music");
            battleScreenMusic[0] = Content.Load<Song>("music/ryu_theme");
            battleScreenMusic[1] = Content.Load<Song>("music/ken_theme");
            battleScreenMusic[2] = Content.Load<Song>("music/chunli_theme");

            titleScreenStartSound = Content.Load<SoundEffect>("button_sounds/title_screen_start");
            instructionScreenReadySound = Content.Load<SoundEffect>("button_sounds/instruction_screen_ready");
            instructionScreenStartSound = Content.Load<SoundEffect>("button_sounds/instruction_screen_start");

            background[0] = Content.Load<Texture2D>("street_fighter_background");
            background[1] = Content.Load<Texture2D>("ken_background");
            background[2] = Content.Load<Texture2D>("chun_li_background");
            ...
            narratorFight = Content.Load<SoundEffect>("narrator_sounds/fight");

            //Title Screen Variables
            globe = Content.Load<Texture2D>("globe");
            titleScreenTitlePic = Content.Load<Texture2D>("title_screen_background");

            //Instruction Screen Variables
            xboxControlsMovementPic = Content.Load<Texture2D>("xboxcontrols_dpad");
            xboxControlsAttacksPic = Content.Load<Texture2D>("xboxcontrols_attacks");

            leftShoulderNotClickedPic = Content.Load<Texture2D>("ShoulderClicks/LeftShoulder_NotClicked");
            leftShoulderClickedPic = Content.Load<Texture2D>("ShoulderClicks/LeftShoulder_Clicked");
            rightShoulderNotClickedPic = Content.Load<Texture2D>("ShoulderClicks/RightShoulder_NotClicked");
            rightShoulderClickedPic = Content.Load<Texture2D>("ShoulderClicks/RightShoulder_Clicked");

            //Battle Screen Variables
            for (int i = 0; i < kenIdlePics.Length; i++) kenIdlePics[i] = Content.Load<Texture2D>("ken/1 idle/ken_idle" + (i + 1));
            for (int i = 0; i < kenWalkingPics.Length; i++) kenWalkingPics[i] = Content.Load<Texture2D>("ken/2 walking/ken_walking" + (i + 1));
            for (int i = 0; i < kenJumpingPics.Length; i++) kenJumpingPics[i] = Content.Load<Texture2D>("ken/3 jumping/ken_jumping" + (i + 1));
            kenCrouchPic = Content.Load<Texture2D>("ken/4 crouch/ken_crouch");
            for (int i = 0; i < kenForwardJumpingPics.Length; i++) kenForwardJumpingPics[i] = Content.Load<Texture2D>("ken/6 forward jump/ken_forward_jump" + (i + 1));

            for (int i = 0; i < kenLowPunchPics.Length; i++) kenLowPunchPics[i] = Content.Load<Texture2D>("ken/7 low punch/ken_low_punch" + (i + 1));
            for (int i = 0; i < kenMediumHighPunchPics.Length; i++) kenMediumHighPunchPics[i] = Content.Load<Texture2D>("ken/8 medium-high punch/ken_medium_high_punch" + (i + 1));

            for (int i = 0; i < kenLowMediumCrouchPunchPics.Length; i++) kenLowMediumCrouchPunchPics[i] = Content.Load<Texture2D>("ken/17 crouch low-medium punch/ken_crouch_low_medium_punch" + (i + 1));
            for (int i = 0; i < kenHighCrouchPunchPics.Length; i++) kenHighCrouchPunchPics[i] = Content.Load<Texture2D>("ken/19 crouch high punch/ken_crouch_high_punch" + (i + 1));

            for (int i = 0; i < kenForwardLowPunchPics.Length; i++) kenForwardLowPunchPics[i] = Content.Load<Texture2D>("ken/9 forward low punch/ken_forward_low_punch" + (i + 1));
            for (int i = 0; i < kenForwardMediumPunchPics.Length; i++) kenForwardMediumPunchPics[i] = Content.Load<Texture2D>("ken/10 forward medium punch/ken_forward_medium_punch" + (i + 1));
            for (int i = 0; i < kenForwardHighPunchPics.Length; i++) kenForwardHighPunchPics[i] = Content.Load<Texture2D>("ken/11 forward high punch/ken_forward_high_punch" + (i + 1));

            for (int i = 0; i < kenLowMediumKickPics.Length; i++) kenLowMediumKickPics[i] = Content.Load<Texture2D>("ken/12 low-medium kick/ken_low_medium_kick" + (i + 1));
            for (int i = 0; i < kenHighKickPics.Length; i++) kenHighKickPics[i] = Content.Load<Texture2D>("ken/13 high kick/ken_high_kick" + (i + 1));

            for (int i = 0; i < kenLowCrouchKickPics.Length; i++) kenLowCrouchKickPics[i] = Content.Load<Texture2D>("ken/20 crouch low kick/ken_crouch_low_kick" + (i + 1));
            for (int i = 0; i < kenMediumCrouchKickPics.Length; i++) kenMediumCrouchKickPics[i] = Content.Load<Texture2D>("ken/21 crouch medium kick/ken_crouch_medium_kick" + (i + 1));
            for (int i = 0; i < kenHighCrouchKickPics.Length; i++) kenHighCrouchKickPics[i] = Content.Load<Texture2D>("ken/22 crouch high kick/ken_crouch_high_kick" + (i + 1));

            for (int i = 0; i < kenForwardLowKickPics.Length; i++) kenForwardLowKickPics[i] = Content.Load<Texture2D>("ken/14 forward low kick/ken_forward_low_kick" + (i + 1));
            for (int i = 0; i < kenForwardMediumKickPics.Length; i++) kenForwardMediumKickPics[i] = Content.Load<Texture2D>("ken/15 forward medium kick/ken_forward_medium_kick" + (i + 1));
            for (int i = 0; i < kenForwardHighKickPics.Length; i++) kenForwardHighKickPics[i] = Content.Load<Texture2D>("ken/16 forward high kick/ken_forward_high_kick" + (i + 1));

            for (int i = 0; i < kenHadoukenPics.Length; i++) kenHadoukenPics[i] = Content.Load<Texture2D>("ken/28 hadouken/ken_hadouken" + (i + 1));
            for (int i = 0; i < kenHadoukenProjectilePics.Length; i++) kenHadoukenProjectilePics[i] = Content.Load<Texture2D>("ken/28 hadouken/ken_projectile_hadouken" + (i + 1));
            for (int i = 0; i < kenTatsumakiPics.Length; i++) kenTatsumakiPics[i] = Content.Load<Texture2D>("ken/29 tatsumaki/ken_tatsumaki" + (i + 1));
            for (int i = 0; i < kenShoryukenPics.Length; i++) kenShoryukenPics[i] = Content.Load<Texture2D>("ken/30 shoryuken/ken_shoryuken" + (i + 1));

            for (int i = 0; i < kenHitPics.Length; i++) kenHitPics[i] = Content.Load<Texture2D>("ken/31 hit/ken_hit" + (i + 1));
            for (int i = 0; i < kenFaceHitPics.Length; i++) kenFaceHitPics[i] = Content.Load<Texture2D>("ken/32 face hit/ken_face_hit" + (i + 1));
            kenCrouchHitPic = Content.Load<Texture2D>("ken/33 crouch hit/ken_crouch_hit");
            for (int i = 0; i < kenKnockdownPics.Length; i++) kenKnockdownPics[i] = Content.Load<Texture2D>("ken/34 knockdown and recover/ken_knockdown" + (i + 1));
            for (int i = 0; i < kenKnockoutPics.Length; i++) kenKnockoutPics[i] = Content.Load<Texture2D>("ken/35 knockout/ken_knockout" + (i + 1));
            for (int i = 0; i < kenVictoryPics.Length; i++) kenVictoryPics[i] = Content.Load<Texture2D>("ken/36 victory/ken_victory" + (i + 1));


            for (int i = 0; i < ryuIdlePics.Length; i++) ryuIdlePics[i] = Content.Load<Texture2D>("ryu/1 idle/ryu_idle" + (i + 1));
            for (int i = 0; i < ryuWalkingPics.Length; i++) ryuWalkingPics[i] = Content.Load<Texture2D>("ryu/2 walking/ryu_walking" + (i + 1));
            for (int i = 0; i < ryuJumpingPics.Length; i++) ryuJumpingPics[i] = Content.Load<Texture2D>("ryu/3 jumping/ryu_jumping" + (i + 1));
            ryuCrouchPic = Content.Load<Texture2D>("ryu/4 crouch/ryu_crouch");
            for (int i = 0; i < ryuForwardJumpingPics.Length; i++) ryuForwardJumpingPics[i] = Content.Load<Texture2D>("ryu/6 forward jump/ryu_forward_jump" + (i + 1));

            for (int i = 0; i < ryuLowPunchPics.Length; i++) ryuLowPunchPics[i] = Content.Load<Texture2D>("ryu/7 low punch/ryu_low_punch" + (i + 1));
            for (int i = 0; i < ryuMediumHighPunchPics.Length; i++) ryuMediumHighPunchPics[i] = Content.Load<Texture2D>("ryu/8 medium-high punch/ryu_medium_high_punch" + (i + 1));

            for (int i = 0; i < ryuLowMediumCrouchPunchPics.Length; i++) ryuLowMediumCrouchPunchPics[i] = Content.Load<Texture2D>("ryu/17 crouch low-medium punch/ryu_crouch_low_medium_punch" + (i + 1));
            for (int i = 0; i < ryuHighCrouchPunchPics.Length; i++) ryuHighCrouchPunchPics[i] = Content.Load<Texture2D>("ryu/19 crouch high punch/ryu_crouch_high_punch" + (i + 1));

            for (int i = 0; i < ryuForwardLowPunchPics.Length; i++) ryuForwardLowPunchPics[i] = Content.Load<Texture2D>("ryu/9 forward low punch/ryu_forward_low_punch" + (i + 1));
            for (int i = 0; i < ryuForwardMediumPunchPics.Length; i++) ryuForwardMediumPunchPics[i] = Content.Load<Texture2D>("ryu/10 forward medium punch/ryu_forward_medium_punch" + (i + 1));
            for (int i = 0; i < ryuForwardHighPunchPics.Length; i++) ryuForwardHighPunchPics[i] = Content.Load<Texture2D>("ryu/11 forward high punch/ryu_forward_high_punch" + (i + 1));

            for (int i = 0; i < ryuLowMediumKickPics.Length; i++) ryuLowMediumKickPics[i] = Content.Load<Texture2D>("ryu/12 low-medium kick/ryu_low_medium_kick" + (i + 1));
            for (int i = 0; i < ryuHighKickPics.Length; i++) ryuHighKickPics[i] = Content.Load<Texture2D>("ryu/13 high kick/ryu_high_kick" + (i + 1));

            for (int i = 0; i < ryuLowCrouchKickPics.Length; i++) ryuLowCrouchKickPics[i] = Content.Load<Texture2D>("ryu/20 crouch low kick/ryu_crouch_low_kick" + (i + 1));
            for (int i = 0; i < ryuMediumCrouchKickPics.Length; i++) ryuMediumCrouchKickPics[i] = Content.Load<Texture2D>("ryu/21 crouch medium kick/ryu_crouch_medium_kick" + (i + 1));
            for (int i = 0; i < ryuHighCrouchKickPics.Length; i++) ryuHighCrouchKickPics[i] = Content.Load<Texture2D>("ryu/22 crouch high kick/ryu_crouch_high_kick" + (i + 1));

            for (int i = 0; i < ryuForwardLowKickPics.Length; i++) ryuForwardLowKickPics[i] = Content.Load<Texture2D>("ryu/14 forward low kick/ryu_forward_low_kick" + (i + 1));
            for (int i = 0; i < ryuForwardMediumKickPics.Length; i++) ryuForwardMediumKickPics[i] = Content.Load<Texture2D>("ryu/15 forward medium kick/ryu_forward_medium_kick" + (i + 1));
            for (int i = 0; i < ryuForwardHighKickPics.Length; i++) ryuForwardHighKickPics[i] = Content.Load<Texture2D>("ryu/16 forward high kick/ryu_forward_high_kick" + (i + 1));

            for (int i = 0; i < ryuHadoukenPics.Length; i++) ryuHadoukenPics[i] = Content.Load<Texture2D>("ryu/28 hadouken/ryu_hadouken" + (i + 1));
            for (int i = 0; i < ryuHadoukenProjectilePics.Length; i++) ryuHadoukenProjectilePics[i] = Content.Load<Texture2D>("ryu/28 hadouken/hadouken_projectile" + (i + 1));
            for (int i = 0; i < ryuTatsumakiPics.Length; i++) ryuTatsumakiPics[i] = Content.Load<Texture2D>("ryu/29 tatsumaki/ryu_tatsumaki" + (i + 1));
            for (int i = 0; i < ryuShoryukenPics.Length; i++) ryuShoryukenPics[i] = Content.Load<Texture2D>("ryu/30 shoryuken/ryu_shoryuken" + (i + 1));

            for (int i = 0; i < ryuHitPics.Length; i++) ryuHitPics[i] = Content.Load<Texture2D>("ryu/31 hit/ryu_hit" + (i + 1));
            for (int i = 0; i < ryuFaceHitPics.Length; i++) ryuFaceHitPics[i] = Content.Load<Texture2D>("ryu/32 face hit/ryu_face_hit" + (i + 1));
            ryuCrouchHitPic = Content.Load<Texture2D>("ryu/33 crouch hit/ryu_crouch_hit");
            for (int i = 0; i < ryuKnockdownPics.Length; i++) ryuKnockdownPics[i] = Content.Load<Texture2D>("ryu/34 knockdown and recover/ryu_knockdown" + (i + 1));
            for (int i = 0; i < ryuKnockoutPics.Length; i++) ryuKnockoutPics[i] = Content.Load<Texture2D>("ryu/35 knockout/ryu_knockout" + (i + 1));
            for (int i = 0; i < ryuVictoryPics.Length; i++) ryuVictoryPics[i] = Content.Load<Texture2D>("ryu/36 victory/ryu_victory" + (i + 1));

            kenBlockPic = Content.Load<Texture2D>("ken/5 block/ken_block_standing");
            kenBlockCrouchPic = Content.Load<Texture2D>("ken/5 block/ken_block_crouching");
            ryuBlockPic = Content.Load<Texture2D>("ryu/5 block/ryu_block_standing");
            ryuBlockCrouchPic = Content.Load<Texture2D>("ryu/5 block/ryu_block_crouching");
            ...
            switch (gameState) {
                ...
                case GameState.TitleScreen:
                    ...
                    if (ticks % 30 == 0) {
                        ...
                        if (startMessageVisible) startMessageVisible = false;
                        else startMessageVisible = true;

                    }

                    if (pad1.Buttons.Start == ButtonState.Pressed || pad2.Buttons.Start == ButtonState.Pressed || kb.IsKeyDown(Keys.Enter)) {

                        titleScreenStartSound.Play();
                        titleToInstructionSeparation++;

                    }

                    if (titleToInstructionSeparation > 0) titleToInstructionSeparation++;

                    if (titleToInstructionSeparation == 60) {
                ...
                case GameState.InstructionScreen:
                    if (MediaPlayer.State == MediaState.Stopped)
                        MediaPlayer.Play(instructionScreenMusic);

                    if (pad1.Buttons.LeftShoulder == ButtonState.Pressed || kb.IsKeyDown(Keys.A)) {

                        if (!pad1LeftShoulderPressed) instructionScreenReadySound.Play();
                        pad1LeftShoulderPressed = true;

                    } else pad1LeftShoulderPressed = false;
                    if (pad1.Buttons.RightShoulder == ButtonState.Pressed || kb.IsKeyDown(Keys.D)) {

                        if (!pad1RightShoulderPressed) instructionScreenReadySound.Play();
                        pad1RightShoulderPressed = true;

                    } else pad1RightShoulderPressed = false;
                    if (pad2.Buttons.LeftShoulder == ButtonState.Pressed || kb.IsKeyDown(Keys.Left)) {

                        if (!pad2LeftShoulderPressed) instructionScreenReadySound.Play();
                        pad2LeftShoulderPressed = true;

                    } else pad2LeftShoulderPressed = false;
                    if (pad2.Buttons.RightShoulder == ButtonState.Pressed || kb.IsKeyDown(Keys.Right)) {

                        if (!pad2RightShoulderPressed) instructionScreenReadySound.Play();
                        pad2RightShoulderPressed = true;

                    } else pad2RightShoulderPressed = false;

                    if (pad1LeftShoulderPressed && pad1RightShoulderPressed) {

                        if (!p1Ready) instructionScreenStartSound.Play();
                        p1Ready = true;

                    }

                    if (pad2LeftShoulderPressed && pad2RightShoulderPressed) {

                        if (!p2Ready) instructionScreenStartSound.Play();
                        p2Ready = true;
                    ...
                    if (p1Ready && p2Ready) {
                        ...
                        instructionToBattleSeparation++;

                        if (instructionToBattleSeparation == 60) {

                            gameState = GameState.BattleScreen;
                            MediaPlayer.Stop();
                            randBattleScreenMusic = rand.Next(0, 3);
                ...
                case GameState.BattleScreen:
                    if (MediaPlayer.State == MediaState.Stopped) {
                        ...
                        narratorFight.Play();
                        MediaPlayer.Play(battleScreenMusic[randBattleScreenMusic]);
                    ...
                    if (kenFacingRight && !endGame) {
                        ...
                        if (kenShoryukenComboStage1 && kenShoryukenCounter <= comboTimeLimit) {
                            ...
                            else if (((kb.IsKeyDown(Keys.R) || kb.IsKeyDown(Keys.T) || kb.IsKeyDown(Keys.Y)) || (pad1.Buttons.X == ButtonState.Pressed || pad1.Buttons.Y == ButtonState.Pressed || pad1.Buttons.RightShoulder == ButtonState.Pressed)) && kenShoryukenComboStage3 && kenActionIsNotRunning) {
                        ...
                        if (ryuShoryukenComboStage1 && ryuShoryukenCounter <= comboTimeLimit) {
                            ...
                            else if ((kb.IsKeyDown(Keys.Left) || pad2.ThumbSticks.Left.X == -1 || pad2.DPad.Left == ButtonState.Pressed) && ryuShoryukenComboStage2) ryuShoryukenComboStage3 = true;

                            else if (((kb.IsKeyDown(Keys.I) || kb.IsKeyDown(Keys.O) || kb.IsKeyDown(Keys.P)) || (pad2.Buttons.X == ButtonState.Pressed || pad2.Buttons.Y == ButtonState.Pressed || pad2.Buttons.RightShoulder == ButtonState.Pressed)) && ryuShoryukenComboStage3 && ryuActionIsNotRunning) {
                    ...
                    if (!kenFacingRight && !endGame) {
                        ...
                        if (ryuShoryukenComboStage1 && ryuShoryukenCounter <= comboTimeLimit) {
                            ...
                            else if ((kb.IsKeyDown(Keys.Right) || pad2.ThumbSticks.Left.X == 1 || pad2.DPad.Right == ButtonState.Pressed) && ryuShoryukenComboStage2) ryuShoryukenComboStage3 = true;
                    ...
                    if ((ken == KenState.Victory || ken == KenState.VictoryReverse || ryu == RyuState.Victory || ryu == RyuState.VictoryReverse)
                        && (pad1.Buttons.Back == ButtonState.Pressed || pad2.Buttons.Back == ButtonState.Pressed || kb.IsKeyDown(Keys.Back))
                        && (kenKnockoutState == kenKnockoutPics.Length - 1 || ryuKnockoutState == ryuKnockoutPics.Length - 1)) {
                        ...
                        titleToInstructionSeparation = 0;
                        instructionToBattleSeparation = 0;
                        p1Ready = false;
                        p2Ready = false;
        ...
        protected override void Draw(GameTime gameTime) {
            ...
            switch (gameState) {
                ...
                case GameState.TitleScreen:
                    spriteBatch.Draw(globe, globeRect, Color.White);
                    spriteBatch.Draw(titleScreenTitlePic, titleScreenTitleRect, Color.White);
                    if (startMessageVisible) spriteBatch.DrawString(textFont, startMessage, startMessageVector, Color.Yellow);
                    break;
                case GameState.InstructionScreen:
                    spriteBatch.DrawString(bigTitleFont, "Controls", new Vector2(310, 10), Color.White);
                    spriteBatch.DrawString(titleFont, "Xbox 360", new Vector2(130, 60), Color.White);
                    spriteBatch.DrawString(titleFont, "Keyboard", new Vector2(550, 60), Color.White);
                    spriteBatch.Draw(xboxControlsMovementPic, xboxControlsMovementRect, Color.White);
                    spriteBatch.Draw(xboxControlsAttacksPic, xboxControlsAttacksRect, Color.White);

                    spriteBatch.DrawString(textFont, "Player 1", new Vector2(450, 100), Color.White);
                    spriteBatch.DrawString(textFont, "WASD - Movement", new Vector2(470, 135), Color.White);
                    spriteBatch.DrawString(textFont, "RTY - Punches", new Vector2(470, 170), Color.White);
                    spriteBatch.DrawString(textFont, "FGH - Kicks", new Vector2(470, 205), Color.White);

                    spriteBatch.DrawString(textFont, "Player 2", new Vector2(450, 255), Color.White);
                    spriteBatch.DrawString(textFont, "Arrow Keys - Movement", new Vector2(470, 290), Color.White);
                    spriteBatch.DrawString(textFont, "IOP - Punches", new Vector2(470, 325), Color.White);
                    spriteBatch.DrawString(textFont, "KL; - Kicks", new Vector2(470, 360), Color.White);

                    if (pad1LeftShoulderPressed) spriteBatch.Draw(leftShoulderClickedPic, p1LeftShoulderRect, Color.White);
                    else spriteBatch.Draw(leftShoulderNotClickedPic, p1LeftShoulderRect, Color.White);

                    if (pad1RightShoulderPressed) spriteBatch.Draw(rightShoulderClickedPic, p1RightShoulderRect, Color.White);
                    else spriteBatch.Draw(rightShoulderNotClickedPic, p1RightShoulderRect, Color.White);

                    if (pad2LeftShoulderPressed) spriteBatch.Draw(leftShoulderClickedPic, p2LeftShoulderRect, Color.White);
                    else spriteBatch.Draw(leftShoulderNotClickedPic, p2LeftShoulderRect, Color.White);

                    if (pad2RightShoulderPressed) spriteBatch.Draw(rightShoulderClickedPic, p2RightShoulderRect, Color.White);
                    else spriteBatch.Draw(rightShoulderNotClickedPic, p2RightShoulderRect, Color.White);

                    if (p1Ready) {

                        spriteBatch.Draw(leftShoulderClickedPic, p1LeftShoulderRect, Color.White);
                        spriteBatch.Draw(rightShoulderClickedPic, p1RightShoulderRect, Color.White);

                    }

                    if (p2Ready) {

                        spriteBatch.Draw(leftShoulderClickedPic, p2LeftShoulderRect, Color.White);
                        spriteBatch.Draw(rightShoulderClickedPic, p2RightShoulderRect, Color.White);

                    }

                    break;
                case GameState.BattleScreen:
                    spriteBatch.Draw(background[randBattleScreenMusic], backgroundRect, Color.White);
            ...
            switch (ken) {
                ...
                case KenState.Victory:
                    ...
                    if (ryuKnockoutState == ryuKnockoutPics.Length - 1) {
                        ...
                        spriteBatch.DrawString(textFont, "Press Back or Backspace to go to Main Menu", new Vector2(200, 180), Color.Yellow);
